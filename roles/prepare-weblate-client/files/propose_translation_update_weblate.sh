...
# Replace /'s in the branch name with -'s to keep the same naming
# rule we followed on Zanata, where /'s were not allowed in the
# version name.
WEBLATE_CATEGORY=${BRANCH//\//-}

SCRIPTSDIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPTSDIR/common_translation_update_weblate.sh"
...
# NOTE: This is expected to call after pulling translations from Weblate.
...
function handle_python_django {
    ...
    if [ -n "$module_names" ]; then
        ...
        propose_releasenotes "$WEBLATE_CATEGORY"
        for modulename in $module_names; do
            ...
            propose_python_django "$modulename" "$WEBLATE_CATEGORY"
        done
    fi
}
...
# Download the current translations from Weblate before we start
# working on them.
pull_from_weblate "$PROJECT" "$WEBLATE_CATEGORY"

case "$PROJECT" in
    api-site|openstack-manuals|security-doc)
        init_manuals "$PROJECT"
        setup_manuals "$PROJECT" "$WEBLATE_CATEGORY"
        propose_manuals
        propose_releasenotes "$WEBLATE_CATEGORY"
        ;;
    training-guides)
        setup_training_guides "$WEBLATE_CATEGORY"
        propose_training_guides
        ;;
    i18n)
        setup_i18n "$WEBLATE_CATEGORY"
        propose_i18n
        ;;
    *)
