#!/bin/bash -e
# Common code used by propose_translation_update_weblate.sh and
...
function setup_manuals {
    ...
    for FILE in ${DocFolder}/*; do
        ...
        if [ ${IS_RST} -eq 1 ] ; then
            ...
            WEBLATE_RULES="$WEBLATE_RULES -r ${WeblateDocFolder}/${DOCNAME}/source/locale/${DOCNAME}.pot ${DocFolder}/${DOCNAME}/source/locale/{locale_with_underscore}/LC_MESSAGES/${DOCNAME}.po"
        else
            # Update the .pot file
            ./tools/generatepot ${DOCNAME}
            if [ -f ${DocFolder}/${DOCNAME}/locale/${DOCNAME}.pot ]; then
                WEBLATE_RULES="$WEBLATE_RULES -r ${WeblateDocFolder}/${DOCNAME}/locale/${DOCNAME}.pot ${DocFolder}/${DOCNAME}/locale/{locale_with_underscore}.po"
            fi
        fi
    done
    ...
    if [[ $project == "openstack-manuals" ]] && [[ $version == "master" ]]; then
        WEBLATE_RULES="$WEBLATE_RULES -r ./releasenotes/source/locale/releasenotes.pot releasenotes/source/locale/{locale_with_underscore}/LC_MESSAGES/releasenotes.po"
    fi

}

# Setup a training-guides project for Weblate
...
# Setup a i18n project for Weblate
function setup_i18n {
    local project=i18n
    local version=${1:-master}
...
# Setup a ReactJS project for Weblate
...
EOF
    ...
    # See if there is an open change in the weblate/translations
...
function send_patch {
    ...
    if [ $(git diff --cached | wc -l) -gt 0 ]; then
        # Commit changes
        git commit -F- -s \
            --trailer="Generated-By:openstack/openstack-zuul-jobs:roles/prepare-weblate-client/files/common_translation_update_weblate.sh" \
            <<EOF
$COMMIT_MSG
EOF
...
# Download the current translations for a project from Weblate.
#
# Every component of the project that belongs to the category matching the
# branch we work on is pulled, and each po file is written to the location
# the repository expects. The path mapping lives in
# download_translations_weblate.py; unlike Zanata there is no per-project
# rules file to generate beforehand, so this can be called once up front.
#
# Weblate has no option to skip files that are barely translated, so as
# with Zanata we download everything and then drop the new files that are
# not translated enough.
#
# $1 - Weblate project slug (defaults to $PROJECT)
# $2 - Weblate category slug (defaults to $WEBLATE_CATEGORY)
function pull_from_weblate {
    local project=${1:-$PROJECT}
    local category=${2:-$WEBLATE_CATEGORY}

    echo "Pulling translations from Weblate:" \
...
function copy_pot {
    local all_modules=$1
    local target=.translation-source/$WEBLATE_CATEGORY/
